...
import type { ColumnConfig } from "@/app/components/shared/types";
...
export function ProjectPage({ projectId, initialTab = "documents" }: Props) {
    ...
    async function handleNewChat() {
        ...
        try {
            ...
            if (id) router.push(`/startups/${projectId}/assistant/chat/${id}`);
        } finally {
    ...
    async function handleCreateReview(
        ...
        try {
            ...
            router.push(`/startups/${projectId}/tabular-reviews/${review.id}`);
        } finally {
    ...
    return (
        <div className="flex-1 overflow-y-auto bg-white flex flex-col h-full">
            <ProjectPageHeader
                ...
                onBackToProjects={() => router.push("/startups")}
                onOpenDocuments={() => router.push(`/startups/${projectId}`)}
            ...
            <div className="min-w-max flex min-h-full flex-col">
                ...
                {tab === "assistant" && (
                    <ProjectAssistantTab
                        ...
                        onOpenChat={(chatId) =>
                            router.push(
                                `/startups/${projectId}/assistant/chat/${chatId}`,
                            )
                        }
                ...
                {tab === "reviews" && (
                    <ProjectReviewsTab
                        ...
                        onOpenReview={(reviewId) =>
                            router.push(
                                `/startups/${projectId}/tabular-reviews/${reviewId}`,
                            )
                        }

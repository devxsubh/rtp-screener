...
export const BUILT_IN_WORKFLOWS: RtpWorkflow[] = [
    ...
    // ─── Compliance screening (Assistant) ─────────────────────────────────────────
    {
        ...
        prompt_md:
            ...
            "Then:\n" +
            "1. Summarize total entities screened and counts by risk level (flagged / review / clear).\n" +
            "2. Highlight the highest-risk owners and any ownership chains through shell companies.\n" +
            "3. Use get_entity_details for any entity the user asks about — never invent match scores.\n" +
    ...
    },
    {
        ...
        prompt_md:
            ...
            "Use get_screening_summary and get_entity_details for facts. Decision support only — not a legal determination.",
        columns_config: null,
    },

    // ─── Cap Table Sanctions Review (tabular) ────────────────────────────────────
    {
        ...
        columns_config: [
            ...
            },
            {
                ...
                name: "Ultimate Owner?",
                format: "yes_no",
                prompt: "Is this entity the ultimate beneficial owner (i.e. it is not owned by another entity in the cap table)? Answer Yes or No.",
            },
            {
                index: 3,
                name: "Ownership Path",
                format: "text",
                prompt: "Describe the full ownership chain from this entity up to the portfolio company. For example: 'BVI HoldCo → Cayman Fund → PortCo'. If the entity is the direct owner, state 'Direct'.",
            },
            {
                index: 4,
                name: "Indirect %",
                format: "text",
                prompt: "What is the effective indirect ownership stake in the portfolio company (%)? State the percentage, or 'N/A' for roster entities with no ownership graph.",
            },
            {
                index: 5,
                name: "Sanctions Match",
                format: "text",
                prompt: "What is the closest match found on the OFAC SDN or UN consolidated sanctions list for this entity? State the matched name, or 'No match' if the entity is clear.",
            },
            {
                index: 6,
                name: "Match Score",
                format: "text",
                prompt: "What is the match confidence score (0–100%) for the closest sanctions list match? State the percentage, or 'N/A' if there is no match.",
            },
            {
                index: 7,
                name: "Source List",
                format: "text",
                prompt: "Which sanctions list produced the highest-scoring match? State the list name (e.g. 'OFAC SDN', 'UN Consolidated'), or 'N/A' if no match.",
            },
            {
                index: 8,
                name: "Risk",
                ...
                prompt: "Based on the match score, what is the risk classification for this entity? Answer 'Flagged' (score ≥ 90%), 'Review' (score 70–89%), or 'Clear' (score < 70% or no match).",
            },
            {
                index: 9,
                name: "Status",
                format: "text",
                prompt: "Leave this column blank — it is for the compliance officer to complete after human review.",
            },
        ],
    },
